...
namespace CIATools_HUD
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void button_build(object sender, EventArgs e)
        {
            ...
            Process.Start("cmd.exe", $"/c cd /d \"{execute_py}\" && py -3 import.py");
        }
        ...
        {
            MessageBox.Show("CIATools HUD\nVersion 5.1\n\nCIABUILDER\nBuild-3\n\nLauncher\nVersion 1.4.2", "Version Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        {
            string execute_py = Path.Combine(rootFolder, "PYSCRIPT");
            Process.Start("cmd.exe", $"/c cd /d \"{execute_py}\" && py -3 compile.py");
        }

        private void sTARTDeletepyToolStripMenuItem_Click(object sender, EventArgs e)
        {
            string execute_py = Path.Combine(rootFolder, "PYSCRIPT");
            Process.Start("cmd.exe", $"/c cd /d \"{execute_py}\" && py -3 delete.py");
        }
